...
const EditPdfResource = ({ resource, onClose, onUpdate }) => {
  ...
  const [pdfFileName, setPdfFileName] = useState(resource.pdfFileName || '');

  useEffect(() => {
    // Set image preview if resource has an imageId
    if (resource.imageId) {
      setImagePreview(getFileUrl(resource.imageId));
    }
  }, [resource]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (image) {
        ...
        if (imageUploadRes && imageUploadRes.$id) {
          updatedResource.imageId = imageUploadRes.$id;
        } else {
      ...
      await updateDoc(resourceRef, {
        ...
        imageId: updatedResource.imageId,
        pdfId: updatedResource.pdfId,
      ...
      setError('Failed to update PDF resource. Please try again.');
    } finally {
      setLoading(false);
    }
  };
...
export default EditPdfResource;
