...
const EditReminder = ({ reminder, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let imageUrl = reminder.imageUrl; // Keep existing image by default
      ...
      console.log("Updating reminder data in Firestore");
      ...
      console.log("Document updated with ID:", reminder.id);

      // Update UI and close modal
      onUpdate({...reminder, ...updatedData});
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition-opacity duration-300">
      <div className="bg-white rounded-lg p-6 w-full max-w-md transform transition-all duration-300">
        <h2 className="text-2xl font-bold mb-4">Edit Reminder</h2>
